...
int	handle_multiplication()
{
	if (g_error)	return 0;
	int result = handle_brackets();
	while (!g_error && g_input[g_pos] == '*')
	{
		g_pos++;
...
{
	if (g_error)	return 0;
	int result = handle_multiplication();
	while (!g_error && g_input[g_pos] == '+')
	{
		g_pos++;
		result += handle_multiplication ();
	}
	return result;
}
